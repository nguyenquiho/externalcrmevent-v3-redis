 ...
 * error returns void. It seems weird to have functions which will either always
 * return true or produce an exception. With such method the return value is
 * never used. Therefore they simply either execute with or without raising an
 * exception.
 *
 */
public class Beanstemc {
    ...
    /**
     * A helper function for reading a response line from the current
     * connection.
     *
    ...
    /**
     * Create and insert a job into the currently used tube (see the "use"
     * method).
     *
     * @param data The payload of the job.
     *
     * @param priority Priority of the job. Smaller priority values will be
     * scheduled before jobs with larger priorities.
     * <em>Do not use priorities larger 2^32-1</em>
     *
     * @param delay The number of seconds to wait before putting the job in the
     * ready queue. The job will be in the "delayed" state during this time.
     *
     * @param ttr Time to run, the number of seconds to allow a worker to run
     * this job. The time is counted from the moment a worker reserves the job.
     * If the worker does not delete, release, or bury the job within ttr
     * seconds, the job will time out and the server will release the job. The
     * minimum ttr is 1. If the client sends 0, the server will silently
     ...
     * @throws dk.safl.beanstemc.BeanstemcException
     ...
     * @throws BeanstemcException
     */
    public Beanstemc use(String tube) throws IOException, BeanstemcException {
     ...
     * response until one becomes available. Once a job is reserved for the
     * client, the client has limited time to run (TTR) before the job times
     * out.
     *
     * When the job times out, the server will put the job back into the ready
     * queue. Both the TTR and the actual time left can be found in response to
     ...
     * reserve command the call will throw BeanstalkcExption with the message
     * "DEADLINE_SOON".
     *
     * @param timeout A timeout value of 0 will result in a reserve call to
     * immediately return either a job a throw a BeanstalkcException with the
     * message "TIMED_OUT". A positive value of timeout will limit the amount of
     ...
     *
     * @return Job The job with the shortest delay left among <em>delayed</em>
     * jobs, in the currently used tube.
     * @throws IOException
     * @throws BeanstemcException
     */
     ...
     *
     * @return Job The first job in the list of <em>buried</em> jobs, in the
     * currently used tube.
     * @throws IOException
     * @throws BeanstemcException
     */
    ...
    /**
     * Removes a job from the server entirely. It is normally used by the client
     * when the job has successfully run to completion. A client can
     * <em>only</em> delete jobs that it has <em>reserved</em>, jobs in state
     * <em>ready</em>, and jobs in state <em>buried</em>.
     *
     ...
     * @throws BeanstemcException
     */
    public Beanstemc delete(long jobId) throws IOException, BeanstemcException {
     ...
     * @see #delete(long)
     */
    public Beanstemc delete(Job job) throws IOException, BeanstemcException {
        return delete(job.getId());
    }

    /**
     * Put a reserved job back into the ready queue (and marks its state as
     * "ready") to be run by any client. It is normally used when the job fails
     * because of a transitory error.
     *
     * @param jobId Is the job id to release.
     * @param priority Is a new priority to assign to the job.
     * @param delay Number of seconds to wait before putting the job in the
     * ready queue. The job will be in the "delayed" state during this time.
     * @return the current object instance.
     * @throws IOException
     * @throws BeanstemcException
     */
    public Beanstemc release(long jobId, long priority, int delay) throws IOException, BeanstemcException {
     ...
     */
    public Beanstemc release(long jobId, long priority) throws IOException, BeanstemcException {
        return release(jobId, priority, DEFAULT_DELAY);
    }
     ...
     * @see #release(long, long, int)
     */
    public Beanstemc release(long jobId) throws IOException, BeanstemcException {
        return release(jobId, DEFAULT_PRIORITY, DEFAULT_DELAY);
    }
     ...
     * @see #release(long, long, int)
     */
    public Beanstemc release(Job job, long priority, int delay) throws IOException, BeanstemcException {
     ...
     * @see #release(long, long, int)
     */
    public Beanstemc release(Job job, long priority) throws IOException, BeanstemcException {
        return release(job.getId(), priority, DEFAULT_DELAY);
    }
     ...
     * @see #release(long, long, int)
     */
    public Beanstemc release(Job job) throws IOException, BeanstemcException {
     ...
     * the server again until a client kicks them with the "kick" command.
     *
     ...
     * @throws BeanstemcException
     */
    public Beanstemc bury(long jobId, long priority) throws IOException, BeanstemcException {
     ...
     * @see #bury(long, long)
     */
    public Beanstemc bury(long jobId) throws IOException, BeanstemcException {
        return bury(jobId, DEFAULT_PRIORITY);
    }
     ...
     * @see #bury(long, long)
     */
    public Beanstemc bury(Job job) throws IOException, BeanstemcException {
     ...
     *
     * A reserve command will take a job from any of the tubes in the watch
     * list. For each new connection, the watch list initially consists of one
     * tube, named "default".
     *
     * @param tube Name of the tube to watch, the name can be no longer than 200
     * bytes. The tube will be created if it does not already exist.
     * @return the current object instance.
     * @throws IOException
     * @throws BeanstemcException
     */
    public Beanstemc watch(String tube) throws IOException, BeanstemcException {
     ...
     * @throws BeanstemcException
     */
    public Beanstemc ignore(String tube) throws IOException, BeanstemcException {
     ...
     *
     * @param bound The upper bound on the number of jobs to kick. The server
    ...
    /**
     * Request more time to work on a job. This is useful for jobs that
     * potentially take a long time, but you still want the benefits of a TTR
     * pulling a job away from an unresponsive worker. A worker may periodically
     * tell the server that it's still alive and processing a job (e.g. it may
     * do this on DEADLINE_SOON).
     *
     * @param jobId
     * @return the current object instance.
     * @throws IOException
     * @throws BeanstemcException
     */
    public Beanstemc touch(long jobId) throws IOException, BeanstemcException {
     ...
     * @throws BeanstemcException
     */
    public Beanstemc pauseTube(String tube, int delay) throws IOException, BeanstemcException {
     ...
     *
     * Only one tube can be in use at a time, changing tube is done by invoking:
     * {@link #use(String)}.
     *
     * @return List of tube names.
     * @throws IOException
